...
class Trainer():
    def __init__(self, net, optimizer, epochs,
        ...
        sample_weights = torch.tensor(sample_weights, dtype=torch.float32) if sample_weights and len(sample_weights)>0 else None
        self.criterion = nn.BCEWithLogitsLoss(weight=sample_weights, reduction='sum')
    ...
    def train(self, train_loader, eval_loader):
        ...
        early_stopping = EarlyStopping(patience=50, path=self.checkpoints_folder + "/best_" + run_name + ".pt")
        ...
        for epoch in range(self.epochs):  # loop over the dataset multiple times
            ...
            for inputs, labels in tqdm(train_loader, total=len(train_loader), desc='Train round', unit='batch', leave=False):  # for each batch
                ...
                if self.use_cuda:
                    ...
                    labels = torch.stack(labels, dim=1).cuda('cuda:%i' %self.gpu_num)
                ...
                train_correct += torch.all(predicted == labels, dim=1).sum().item()  # how much corrected seen so far

                train_y_pred = torch.cat((train_y_pred, predicted.cpu()))
                train_y_true = torch.cat((train_y_true, labels.cpu()))
            ...
            with torch.no_grad():
                 for inputs, labels in tqdm(eval_loader, total=len(eval_loader), desc='Val round', unit='batch', leave=False):   # for each batch
                    ...
                    predicted = torch.round(torch.nn.functional.sigmoid(eval_outputs))
                    
                    total += labels.size(0)  # how much samples seen so far
                    correct += torch.all(predicted == labels, dim=1).sum().item()  # how much corrected seen so far
