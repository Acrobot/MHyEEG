...
class DatasetCreator:
    ...
    def __init__(self, preproc_data, label_kinds,
    ...
    def save_to_list(self):
        ...
        for dir in tqdm(sub_dir, desc='Reading data'):  # for each subject/folder
            ...
            # return Dataframe
            all_labels = np.dstack(tuple(
                np.genfromtxt(
                    os.path.join(dir, f'labels_felt{x}.csv'), delimiter=','
            ...
            for i, id in enumerate(tqdm(id_trials, desc=f'Subject {subj}')):
                ...
                for j, k in zip(range(0, n_points_block_gaze - overlap_step_gaze, n_points_sample_gaze - overlap_step_gaze),
                    ...
                    label = all_labels[:, i].flatten().copy()
                    if np.any(label <= 0) or np.any(label > 9):  # sanity check on the labels
                        print("\033[93mLabels went wrong at subject: {}, sample: {}!\033[0m".format(subj, id))

                    threshold = 5  # As in the paper
                    label[np.where(label < threshold)] = 0
                    label[np.where(label >= threshold)] = 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--augment-data', type=bool, action=argparse.BooleanOptionalAction, default=False)
    args = parser.parse_args()

    label_kinds = ("Arsl", "Vlnc")
    print("Creating dataset")

    set_seed(args.seed)

    d = DatasetCreator(args.preproc_data_path, label_kinds, physio_f = 256, gaze_f = 60, block_len = 34, sample_len=34, overlap = 0, verbose=args.verbose)  # create object
